...
static void vec_malloc(struct vec *self, align_t align, index_t len) {
  assert(align > 0);
  assert(0 <= len);
  self->box = box_data_new(align * len);
  array_init(vec_inner(self), align, box_data_ptr(self->box));
}
static void vec_free(struct vec *self) {
  box_data_delete(self->box);
...
index_t vec_capacity(const struct vec *self) {
  return box_data_size(self->box) / array_align(vec_inner(self));
}
