...
static bool_t convert_return_statement(struct convert_return_extra *,
                                       struct json *);
static void convert_return_statement_list(struct json_map *map) {
  struct convert_return_extra *self = json_map_extra(map);
  struct json *json = json_map_val(map);
  if (convert_return_statement(self, json)) {
    self->must_return = true;
    json_map_finish(map);
  }
}
static bool_t convert_return_compound_statement(
...
static bool_t convert_return_selection_statement(
    struct convert_return_extra *self, struct json *json) {
  if (json_has(json, SYMBOL_IF)) {
    bool_t then_must_return = convert_return_statement(
        self, json_obj_get(json, SYMBOL_THEN_STATEMENT));
    if (json_has(json, SYMBOL_ELSE)) {
      bool_t else_must_return = convert_return_statement(
          self, json_obj_get(json, SYMBOL_ELSE_STATEMENT));
      return then_must_return && else_must_return;
    }
  }
  return false;
}
static bool_t convert_return_jump_statement(struct convert_return_extra *self,
                                            struct json *json) {
  UTIL_UNUSED(self);
  return json_has(json, SYMBOL_RETURN);
}
static bool_t convert_return_statement(struct convert_return_extra *self,
                                       struct json *json) {
  bool_t must_return = false;
  if (false) {
  } else if (json_has(json, SYMBOL_COMPOUND_STATEMENT)) {
    json = json_obj_get(json, SYMBOL_COMPOUND_STATEMENT);
    must_return = convert_return_compound_statement(self, json);
  } else if (json_has(json, SYMBOL_SELECTION_STATEMENT)) {
    json = json_obj_get(json, SYMBOL_SELECTION_STATEMENT);
    must_return = convert_return_selection_statement(self, json);
  } else if (json_has(json, SYMBOL_JUMP_STATEMENT)) {
    json = json_obj_get(json, SYMBOL_JUMP_STATEMENT);
    must_return = convert_return_jump_statement(self, json);
  }
  if (must_return) {
...
static void convert_return_visitor(struct json_visitor *visitor,
  ...
  if (json_has(json, SYMBOL_COMPOUND_STATEMENT)) {
    convert_return_compound_statement(
        NULL, json_obj_get(json, SYMBOL_COMPOUND_STATEMENT));
  } else {
