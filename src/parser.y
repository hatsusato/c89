...
: postfix-expression "[" expression "]" {
  $$ = ast_new_tag(AST_POSTFIX_ARRAY, 2);
  ast_append($$, $1);
  ast_append($$, $3);
}
;
postfix-call
: postfix-expression "(" ")" {
  $$ = ast_new_tag(AST_POSTFIX_CALL, 1);
  ast_append($$, $1);
}
;
postfix-call-args
: postfix-expression "(" expression ")" {
  $$ = ast_new_tag(AST_POSTFIX_CALL_ARGS, 2);
  ast_append($$, $1);
  ast_append($$, $3);
}
;
...
: postfix-expression "." identifier {
  $$ = ast_new_tag(AST_POSTFIX_MEMBER, 2);
  ast_append($$, $1);
  ast_append($$, $3);
}
;
postfix-arrow
: postfix-expression "->" identifier {
  $$ = ast_new_tag(AST_POSTFIX_ARROW, 2);
  ast_append($$, $1);
  ast_append($$, $3);
}
;
postfix-incr
: postfix-expression "++" {
  $$ = ast_new_tag(AST_POSTFIX_INCR, 1);
  ast_append($$, $1);
}
;
postfix-decr
: postfix-expression "--" {
  $$ = ast_new_tag(AST_POSTFIX_DECR, 1);
  ast_append($$, $1);
}
;
unary-expression
