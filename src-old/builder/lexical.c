...
void builder_typedef_identifier(Builder *builder, Sexp *ast) {
  UTILITY_ASSERT(SYNTAX_TYPEDEF_IDENTIFIER == sexp_get_tag(ast));
  UTILITY_UNUSED(builder);
  UTILITY_ASSERT(0);
}
void builder_floating_constant(Builder *builder, Sexp *ast) {
  UTILITY_ASSERT(SYNTAX_FLOATING_CONSTANT == sexp_get_tag(ast));
  UTILITY_UNUSED(builder);
  UTILITY_ASSERT(0);
}
void builder_integer_constant(Builder *builder, Sexp *ast) {
  const char *symbol;
  UTILITY_ASSERT(SYNTAX_INTEGER_CONSTANT == sexp_get_tag(ast));
  ast = sexp_at(ast, 1);
...
void builder_enumeration_constant(Builder *builder, Sexp *ast) {
  UTILITY_ASSERT(SYNTAX_ENUMERATION_CONSTANT == sexp_get_tag(ast));
  UTILITY_UNUSED(builder);
  UTILITY_ASSERT(0);
}
void builder_character_constant(Builder *builder, Sexp *ast) {
  UTILITY_ASSERT(SYNTAX_CHARACTER_CONSTANT == sexp_get_tag(ast));
  UTILITY_UNUSED(builder);
  UTILITY_ASSERT(0);
}
void builder_string_literal(Builder *builder, Sexp *ast) {
  UTILITY_ASSERT(SYNTAX_STRING_LITERAL == sexp_get_tag(ast));
  UTILITY_UNUSED(builder);
  UTILITY_ASSERT(0);
}
